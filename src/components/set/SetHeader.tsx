...
export function SetHeader({ set }: SetHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row gap-6 items-start">
      {/* Image */}
      <div className="flex-shrink-0 w-32 h-32 sm:w-40 sm:h-40 rounded-brick overflow-hidden bg-navy-50 border border-cream/10 flex items-center justify-center">
        {set.setImgUrl ? (
          <img
            src={set.setImgUrl}
            alt={set.name}
      ...
      <div className="flex-1 space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-mono text-xs text-lego-yellow/70 bg-lego-yellow/10 border border-lego-yellow/20 px-2 py-0.5 rounded-full">
            {set.setNum}
          </span>
        ...
        <div className="flex items-center gap-4 flex-wrap">
          <div className="flex items-center gap-1.5 text-sm text-cream/50 font-body">
